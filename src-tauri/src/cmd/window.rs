...
pub fn close_all_prompt_windows<R: Runtime>(
    ...
    // Spawn async task to close windows after command returns
    tauri::async_runtime::spawn(async move {
        // Small delay to ensure the command response is sent back to frontend first
        // This prevents deadlock when closing the calling window
    ...
    // Return immediately - windows will be closed asynchronously
    Ok(())
}
