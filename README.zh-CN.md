...
[![Tauri](https://img.shields.io/badge/Tauri-2.11-blue?logo=tauri)](https://tauri.app/)
...
- **多语言支持**：内置英文、中文和其他 8 种常用语言建议，易于自定义
...
- **音量控制**：根据您的喜好调整音频音量
- **预览**：在保存之前测试声音
- **双重播放**：音频在休息开始和自然结束时都会播放，帮助您感知转换时刻
...
- **自动暂停**：检测系统空闲时间并暂停调度器
- **勿扰模式检测**：当启用勿扰模式/专注辅助时自动暂停休息（Windows、macOS、Linux）
- **应用程序排除**：基于活动应用程序暂停或恢复调度器（例如，演示时暂停，编码时恢复）
...
- **超低内存占用**：后台运行时仅使用约 10MB，得益于动态窗口创建和延迟加载的 Vue 组件
...
- **Windows**：`installer.exe` 安装程序或 `portable.zip`（x86_64）
